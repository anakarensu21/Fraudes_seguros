...
pander(summary_dfd)
```
En el mes y año en el que se presentaron menos fraudes, se presentaron un total de 319 incidentes. Mientras que el mayo reporte en un mes fue de 1546, con un promedio de 603.9 fraudes al mes. 
...
pander(summary_dfs)
```
La tabla anterior muestra la cantidad de cada tipo de actividad sospechosa registrada a través de los años. Es importante mencionar que la primera tabla está resumida, solo muestra los primeros 10 diferentes tipos de actividades sospechosas, pues hay una gran variedad registrada.
...
Destacan como principal instrumento utilizado para el fraude las transferencias electrónicas pues son, por mucho, el método más común, seguidas por los cheques personales o comerciales. Instrumentos como los cheques de viajero y la moneda extranjera tienen una frecuencia mucho menor.
\newline
...
Para entender el comportamiento temporal de los fraudes, i.e. la distribución de los datos, se realizaron los siguientes análisis utilizando la paquetería ggplot2:
...
El modelo propuesto se basa en la simulación de Monte Carlo para analizar la evolución de fraudes en compañías aseguradoras a lo largo del tiempo. Se generarán trayectorias aleatorias de eventos de fraude considerando la distribución de probabilidad de ocurrencia en función de dos variables clave:

* Tipo de producto asegurado: Se modelarán diferentes categorías de seguros (vida, automóvil, salud, etc.), cada una con su propia distribución de probabilidad de fraude.

* Instrumento de pago utilizado: Se analizarán los distintos métodos de pago empleados en los fraudes (tarjetas de crédito, transferencias bancarias, efectivo, etc.), asignando una probabilidad de ocurrencia a cada uno.
...
1.	La ocurrencia de fraudes sigue una distribución de probabilidad conocida poisson, debido a que la variable representa el nùmero de fraudes en el tiempo.
...
* *_Tasa de fraude diaria promedio por mes:_* Se estima a partir del número de fraudes registrados por mes dividido entre el nùmero de dìas hábiles.
...
Para la segunda fase del proyecto, se propone el siguiente plan de simulaciones:

1.	*_Preparación de datos:_* Limpieza y preprocesamiento de los datos históricos de fraude.

2.	*_Definición de distribuciones:_* Ajuste de las distribuciones de probabilidad para cada variable relevante.

3.	*_Implementación de la simulación Monte Carlo:_* Generación de múltiples trayectorias aleatorias de fraudes.

4.	*_Análisis de resultados:_* Evaluación de patrones y tendencias mediante gráficos y métricas estadísticas.

5.	*_Comparación de escenarios:_* Simulación de distintos entornos para evaluar la variabilidad en la ocurrencia de fraudes.
